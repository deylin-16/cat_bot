...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        global.db.data.chats[chatJid] ||= { isBanned: false, sAutoresponder: '', modoadmin: false, welcome: true, autoresponder: false, antiLink: true, autoresponder2: false, per: [], welcomeMsg: '¡Bienvenido/a!', primaryBot: '' };
        ...
        if (m.isGroup) {
            ...
            [senderLid, botLid] = await Promise.all([getLidFromJid(m.sender, conn), getLidFromJid(botJid, conn)]);
            ...
            senderLid = m.sender; botLid = conn.user.jid; botJid = conn.user.jid;
        ...
        for (const name in global.plugins) {
            ...
            if (typeof plugin.all === 'function') {
                try { await plugin.all.call(conn, m, { chatUpdate, __dirname: ___dirname, __filename }); } catch (e) { if (!(e instanceof TypeError && e.message.includes('user'))) console.error(e); }
            }
            ...
            const checkPermissions = (perm) => ({
                rowner: isROwner, owner: isOwner, group: m.isGroup, botAdmin: isBotAdmin, admin: isAdmin, private: !m.isGroup, restrict: !opts['restrict'], subBot: isSubAssistant || isROwner
            }[perm]);
            ...
            } catch (e) { m.error = e; m.reply(format(e)); } finally {
                if (typeof plugin.after === 'function') {
                    try { await plugin.after.call(conn, m, { usedPrefix, noPrefix, args, command, text, conn, participants, groupMetadata, user, isROwner, isOwner, isRAdmin, isAdmin, isBotAdmin, isSubAssistant, chatUpdate, __dirname: ___dirname, __filename }); } catch (e) { console.error(e) }
                }
            }
        }
