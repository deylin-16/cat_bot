...
export async function handler(chatUpdate) {
    ...
    // Lógica de Prioridad (Evita que varios bots respondan lo mismo)
    ...
    try {
        ...
        // --- PROCESAMIENTO DE PREFIJO ---
        const _prefix = global.prefix || /^[#!./]/
        const isRegExp = _prefix instanceof RegExp
        const match = isRegExp ? _prefix.exec(m.text) : Array.isArray(_prefix) ? _prefix.find(p => m.text.startsWith(p)) : m.text.startsWith(_prefix) ? _prefix : null
        
        let usedPrefix = isRegExp ? (match ? match[0] : '') : (match || '')
