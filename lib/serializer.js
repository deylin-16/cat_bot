...
export const smsg = async (conn, m) => {
    ...
    if (m.key) {
        ...
        m.sender = await getRealJid(conn, m);
    }
    ...
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            const quotedRaw = { key: { remoteJid: m.chat }, participant: m.msg.contextInfo.participant };
            m.quoted.sender = await getRealJid(conn, quotedRaw);
