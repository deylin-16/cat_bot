...
export const smsg = (conn, m) => {
    ...
    if (m.key) {
        ...
        if (rawSender.endsWith('@lid') && m.isGroup) {
            const metadata = conn.groupMetadataCache?.get(m.chat) || {};
            const participant = (metadata.participants || []).find(p => p.id === rawSender);
            if (participant?.phoneNumber) {
                rawSender = participant.phoneNumber.includes('@') ? participant.phoneNumber : participant.phoneNumber + '@s.whatsapp.net';
            }
        }
    ...
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            let qSender = m.msg.contextInfo.participant;
            if (qSender.endsWith('@lid') && m.isGroup) {
                const metadata = conn.groupMetadataCache?.get(m.chat) || {};
                const participant = (metadata.participants || []).find(p => p.id === qSender);
                if (participant?.phoneNumber) {
                    qSender = participant.phoneNumber.includes('@') ? participant.phoneNumber : participant.phoneNumber + '@s.whatsapp.net';
                }
            }
