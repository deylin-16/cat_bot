...
export const smsg = async (conn, m) => {
    ...
    if (m.message) {
        ...
        m.download = () => conn.downloadM(m.msg, m.mtype?.replace('Message', '') || 'image');
        ...
        if (m.quoted) {
            ...
            m.quoted.download = () => conn.downloadM(m.quoted.msg, m.quoted.mtype?.replace('Message', '') || 'image');
        }
    }
    ...
    conn.getName = (jid) => {
        ...
        return id === '0@s.whatsapp.net' ? 'WhatsApp' : id.split('@')[0] || 'Usuario';
    };
    ...
    conn.reply = (jid, text = '', quoted, options) => {
        ...
        return conn.sendMessage(jid, { text: text || '', mentions, ...options }, { quoted: quoted || m });
    };
