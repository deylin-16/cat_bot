...
export const smsg = (conn, m) => {
    ...
    // FUNCIONES DE COMPATIBILIDAD NATIVAS
    m.reply = (text, chat = m.chat, options = {}) => conn.sendMessage(chat, { text }, { quoted: m, ...options });
    
    m.react = (emoji) => conn.sendMessage(m.chat, { react: { text: emoji, key: m.key } });

    conn.reply = (jid, text, quoted, options) => conn.sendMessage(jid, { text }, { quoted, ...options });
